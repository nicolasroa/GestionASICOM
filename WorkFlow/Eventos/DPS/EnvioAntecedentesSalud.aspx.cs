...
{
    public partial class EnvioAntecedentesSalud : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                CargarAcciones();
                NegRegistroAntecedentesSalud.IndEnvio = true;
        ...
        private bool ValidarEvento()
        {
            try
            {
                ...
                if (oResultado.ResultadoGeneral)
                {
                    if (oResultado.Lista.Where(p => p.FechaIngresoDPS == null && p.PorcentajeDesgravamen > 0).Count() > 0)
                    {
                        var Participante = new ParticipanteInfo();
                        Participante = oResultado.Lista.FirstOrDefault(p => p.FechaIngresoDPS == null && p.PorcentajeDesgravamen > 0);

                        Controles.MostrarMensajeAlerta("Falta completar los datos de los Antecedentes de Salud del Participante " + Participante.NombreCliente);
                        return false;
                    }
                }
                else
