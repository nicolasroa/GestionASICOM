...
{
    public partial class SolicitudEETT : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                CargarInfoSolicitud();
                DatosDocumentosEstudioTitulo.DesplegarDatosEstudioTitulo(false);
        ...
        private bool ValidarDocumentos()
        {
            try
            {
                ...
                if (rAntecedentes.ResultadoGeneral)
                {
                    ...
                    if (rAntecedentes.Lista.Where(d => d.DescripcionEstado == null).Count() > 0)
                    {
                        var Tasacion = new TasacionInfo();
                        Controles.MostrarMensajeAlerta("Debe Asgirnarle un Estado al Documento: " + rAntecedentes.Lista.FirstOrDefault(d => d.DescripcionEstado == null).DescripcionDocumento);
                        return false;
                    }
        ...
        private void CargarInfoSolicitud()
        {
            try
            {
                ...
                txtFechaSolicitudEETT.Text = NegSolicitudes.objSolicitudInfo.FechaSolicitudEstudioTitulo == null ? "" : NegSolicitudes.objSolicitudInfo.FechaSolicitudEstudioTitulo.Value.ToShortDateString();
            }
            catch (Exception Ex)
            {
                if (Constantes.ModoDebug == true)
        ...
        private bool ActualizaSolicitud()
        {
            ...
            try
            {
                ...
                //    return false;
                //}
                if (txtFechaSolicitudEETT.Text == "")
                {
                    Controles.MostrarMensajeAlerta("Debe ingresar una Fecha de Solicitud");
                    return false;
                }
                ...
                NegSolicitudes.objSolicitudInfo.FabricaAbogado_Id = int.Parse(ddlFabAbogados.SelectedValue);
                //NegSolicitudes.objSolicitudInfo.Abogado_Id = int.Parse(ddlAbogado.SelectedValue);
                NegSolicitudes.objSolicitudInfo.FechaSolicitudEstudioTitulo = DateTime.Parse(txtFechaSolicitudEETT.Text);
        ...
        private void GenerarReportePDF()
        {
            try
            {
                ...
                string urlDocumento = "~/Reportes/PDF/DocumentacionEstudioTitulo.aspx";
                string ContenidoHtml = "";
                string Archivo = "DocumentacionEstudioTitulo_Solicitud_" + NegSolicitudes.objSolicitudInfo.Id.ToString() + ".pdf";
                Pdf.NombreDocumentoPDF = Archivo;
                Pdf.ModuloDocumentoPDF = "Documentación EETT";

                int cont = 1;

                foreach (TasacionInfo oInfo in NegPropiedades.lstPropiedadesEETT)
                {
                    oInfo.flagPDF = 0;
