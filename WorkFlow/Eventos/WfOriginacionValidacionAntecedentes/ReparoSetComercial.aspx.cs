...
{
    public partial class ReparoSetComercial : System.Web.UI.Page
    {
        ...
        private void CargarDocumentos()
        {
            try
            {
                ...
                {
                    Controles.CargarGrid(ref gvDocumentosRequeridos, rDocumentos.Lista.OrderByDescending(d => d.EstadoRevision_Id).ToList<RegistroDocumentosOriginacionComercialInfo>(), new string[] { "Documento_Id", "Solicitud_Id" });
                    var Lista = new List<TablaInfo>();
                    ...
                    foreach (GridViewRow Row in gvDocumentosRequeridos.Rows)
                    {
                        ...
                        rblEstadoDocumento.SelectedValue = oDocumentos.EstadoRevision_Id.ToString();
                        if (oDocumentos.EstadoRevision_Id == 4)
                            Row.BackColor = System.Drawing.Color.IndianRed;
                        if (oDocumentos.EstadoRevision_Id == 1)
                            Row.BackColor = System.Drawing.Color.Green;
                        if (oDocumentos.EstadoRevision_Id == 2)
            ...
            try
            {
                ...
                foreach (GridViewRow Row in gvDocumentosRequeridos.Rows)
                {
                    ...
                    if (rblEstadoDocumento.SelectedItem != null)
                    {
                        ...
                        oDocumentos.EstadoRevision_Id = int.Parse(rblEstadoDocumento.SelectedItem.Value);
