...
{
    public partial class ConfeccionMinutaComplementaria : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            Anthem.Manager.Register(Page);
            Controles.EjecutarJavaScript("InicioEvento();");
            if (!Page.IsPostBack)
            {
                CargarAcciones();
            }
        ...
        public void ProcesarEvento()
        {
            try
            {
                ...
                if (oAccion.Sentido_Id == NegTablas.IdentificadorMaestro("SENTIDOS_WF", "A"))//AVANZAR
                {
                    ...
                    if (DateTime.TryParse(txtFechaConfeccionMinutaComplementaria.Text, out Fecha))
                    {
                        NegSolicitudes.objSolicitudInfo.FechaConfeccionMinutaComplementaria = Fecha;
                        if (!NegSolicitudes.ActualizarSolicitud(NegSolicitudes.objSolicitudInfo))
                        {
                            Controles.MostrarMensajeError("Error al Grabar Fecha");
                            return;
                        }
                    }
                    else
                    {
                        Controles.MostrarMensajeAlerta("Debe Ingresar una Fecha");
                        return;
                    }
                }
                else if (oAccion.Sentido_Id == NegTablas.IdentificadorMaestro("SENTIDOS_WF", "F"))//FINALIZAR
