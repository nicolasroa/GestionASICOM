...
{
    public partial class SolicitudTasacionPrevia : System.Web.UI.Page
    {
        ...
        private void CargarInstitucionesFinancieras()
        {
            try
            {
                ...
                if (oResultado.ResultadoGeneral)
                {
                    Controles.CargarCombo(ref ddlInstitucionTasacionPrevia, oResultado.Lista, Constantes.StringId, Constantes.StringDescripcion, "-- Institución Financiera --", "-1");
                }
                else
                {
                    Controles.MostrarMensajeError(oResultado.Mensaje);
        ...
        private bool ValidarPropiedades()
        {
            try
            {
                ...
                if (NegPropiedades.lstPropiedadesTasaciones.Count(t => t.FechaTasacionPrevia == null && t.IndPropiedadPrincipal == true) != 0)
                {
        ...
        private void CargarInfoTasacion(TasacionInfo oTasacion)
        {
            try
            {
                if (oTasacion != null)
                {
                    ...
                    txtFechaTasacionPrevia.Text = oTasacion.FechaTasacionPrevia == null ? "" : oTasacion.FechaTasacionPrevia.Value.ToShortDateString();
                    ddlInstitucionTasacionPrevia.SelectedValue = oTasacion.InstitucionTasacionPrevia_Id.ToString();
                    txtNombreTasadorTasacionPrevia.Text = oTasacion.NombreTasadorTasacionPrevia;
        ...
        private void GrabarInfoTasacion()
        {
            try
            {
                ...
                oTasacion.FechaTasacionPrevia = DateTime.Parse(txtFechaTasacionPrevia.Text);
                oTasacion.NombreTasadorTasacionPrevia = txtNombreTasadorTasacionPrevia.Text;
                oTasacion.InstitucionTasacionPrevia_Id = int.Parse(ddlInstitucionTasacionPrevia.SelectedValue);
        ...
        private void LimpiarInfoTasacion()
        {
            ...
            txtFechaTasacionPrevia.Text = "";
            ddlInstitucionTasacionPrevia.ClearSelection();
            txtNombreTasadorTasacionPrevia.Text = "";
        ...
        private bool ValidarInfoTasacion()
        {
            try
            {
                ...
                if (txtFechaTasacionPrevia.Text == "")
                {
                    Controles.MostrarMensajeAlerta("Debe Ingresar una Fecha de Solicitud de Tasación");
                    return false;
                }
                if (txtNombreTasadorTasacionPrevia.Text == "")
                {
                    Controles.MostrarMensajeAlerta("Debe Ingresar el nombre del Tasador de la Tasación Previa");
                    return false;
                }
