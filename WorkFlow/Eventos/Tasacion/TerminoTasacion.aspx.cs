...
{
    public partial class TerminoTasacion : System.Web.UI.Page
    {
        ...
        public void ProcesarEvento()
        {
            try
            {
                ...
                if (oAccion.Sentido_Id == NegTablas.IdentificadorMaestro("SENTIDOS_WF", "A"))//AVANZAR
                {
                    ...
                    if (!ActualizarTasaciones()) return;
            ...
            try
            {
                ...
                if (rPropiedad.ResultadoGeneral)
                {
                    ...
                    if (rTasacion.ResultadoGeneral)
                    {
                        ...
                        NegSolicitudes.objSolicitudInfo.Inmobiliaria_Id = int.Parse(ddlInmobiliaria.SelectedValue);
                        NegSolicitudes.objSolicitudInfo.Proyecto_Id = int.Parse(ddlProyecto.SelectedValue);
                        NegSolicitudes.objSolicitudInfo.IndDfl2 = chIndDfl2.Checked;
        ...
        private void CargarDatosSolicitud()
        {
            ...
            try
            {
                ...
                if (NegSolicitudes.objSolicitudInfo != null)
                {
                    ...
                    ddlInmobiliaria.SelectedValue = NegSolicitudes.objSolicitudInfo.Inmobiliaria_Id.ToString();
                    CargaComboProyecto(NegSolicitudes.objSolicitudInfo.Inmobiliaria_Id);
                    ddlProyecto.SelectedValue = NegSolicitudes.objSolicitudInfo.Proyecto_Id.ToString();
                    chIndDfl2.Checked = NegSolicitudes.objSolicitudInfo.IndDfl2;
        ...
        private bool ActualizarTasaciones()
        {
            try
            {
                var nTasaciones = new NegPropiedades();
                var rTasaciones = new Resultado<TasacionInfo>();
                var oTasacion = new TasacionInfo();
                foreach (var tasacion in NegPropiedades.lstPropiedadesTasaciones)
                {
                    tasacion.EstadoTasacion_Id = (int)NegTablas.IdentificadorMaestro("ESTADOS_FLUJOS_PARALELOS", "T");// Terminada
                    rTasaciones = nTasaciones.GuardarTasacion(tasacion);
                ...
                oTasacion.Solicitud_Id = NegPropiedades.lstPropiedadesTasaciones.FirstOrDefault().SolicitudTasacion_Id;
                rTasaciones = nTasaciones.BuscarTasacion(oTasacion);
                if (!rTasaciones.ResultadoGeneral)
                {
                    Controles.MostrarMensajeError(rTasaciones.Mensaje);
                    return false;
                }
                foreach (var tasacionInfo in rTasaciones.Lista)
                {
                    tasacionInfo.EstadoTasacion_Id = (int)NegTablas.IdentificadorMaestro("ESTADOS_FLUJOS_PARALELOS", "T");// Terminada
                    rTasaciones = nTasaciones.GuardarTasacion(tasacionInfo);
                    if (!rTasaciones.ResultadoGeneral)
                    {
                        Controles.MostrarMensajeError(rTasaciones.Mensaje);
                        return false;
