...
{
    public partial class ConfeccionEscritura : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            Anthem.Manager.Register(Page);
            Controles.EjecutarJavaScript("InicioBorrador();");
            if (!Page.IsPostBack)
            {
        ...
        private void CargarInfoSolicitud()
        {
            try
            {
                ...
                txtFechaBorrador.Text = NegSolicitudes.objSolicitudInfo.FechaBorradorEscritura == null ? "" : NegSolicitudes.objSolicitudInfo.FechaBorradorEscritura.Value.ToShortDateString();
                txtFechaConfeccionEscritura.Text = NegSolicitudes.objSolicitudInfo.FechaConfeccionEscritura == null ? "" : NegSolicitudes.objSolicitudInfo.FechaConfeccionEscritura.Value.ToShortDateString();
            }
            catch (Exception Ex)
            {
                if (Constantes.ModoDebug == true)
        ...
        private bool ActualizaSolicitud()
        {
            ...
            try
            {
                ...
                NegSolicitudes.objSolicitudInfo.FechaConfeccionEscritura = DateTime.Parse(txtFechaConfeccionEscritura.Text);
