...
{
    public partial class RevisionAbogado : System.Web.UI.Page
    {

        protected void Page_Load(object sender, EventArgs e)
        {
            Anthem.Manager.Register(Page);
            Controles.EjecutarJavaScript("InicioFechas();");
            if (!Page.IsPostBack)
            {
        ...
        private void CargarInfoSolicitud()
        {
            try
            {
                ...
                txtFechaRevisionAbogado.Text = NegSolicitudes.objSolicitudInfo.FechaRevisionAbogado == null ? "" : NegSolicitudes.objSolicitudInfo.FechaRevisionAbogado.Value.ToShortDateString();
            }
            catch (Exception Ex)
            {
                if (Constantes.ModoDebug == true)
        ...
        private bool ActualizaSolicitud()
        {
            ...
            try
            {
                ...
                if (txtFechaRevisionAbogado.Text == "")
                {
                    Controles.MostrarMensajeAlerta("Debe Ingresar una Fecha de Revision Abogado.");
                    return false;
                }
                ...
                NegSolicitudes.objSolicitudInfo.FechaRevisionAbogado = DateTime.Parse(txtFechaRevisionAbogado.Text);
        ...
        public void CargarParticipantes()
        {
            try
            {
                ...
                if (oResultado.ResultadoGeneral)
                {
                    Controles.CargarGrid(ref GvParticipantes, NegParticipante.lstParticipantes, new string[] { "Id", });
                }
                else
                {
                    Controles.MostrarMensajeError(oResultado.Mensaje);
