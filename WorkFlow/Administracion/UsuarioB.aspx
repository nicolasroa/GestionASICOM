<%@ Page Title="" Language="C#" MasterPageFile="~/Master/Site.Master" AutoEventWireup="true"
    CodeBehind="UsuarioB.aspx.cs" Inherits="WebSite.Administracion.UsuarioB" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">


    <div class="row spacenavmax">
        <div class="col-md-12">
            <h4>
                <span class="titulo">
                    <%--<img src="../Img/icons/favicon_xsmall.png" />--%>Maestro de Usuarios
                </span>
                <br>
            </h4>
           <%-- <div style="width: 100%;">
                <img style="width: 100%; height: 10px" src="../Img/LineaAsicom.png" />
            </div>--%>
        </div>
    </div>

    <div id="accordion">
        <h2 id="hBusqueda">
            <a id="lnkBusqueda" href="#">Búsqueda</a>
        </h2>
        <div>
            <div class="row">
                <div class="col-lg-4">
                    <div class='form-group'>
                        <label for="txtNombre" class="text-left">Nombre</label>
                        <anthem:TextBox CssClass="form-control" AutoUpdateAfterCallBack="true" ID="txtNombre" runat="server" TabIndex="1"></anthem:TextBox>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class='form-group'>
                        <label for="txtRut" class="text-left">Rut</label>
                        <anthem:TextBox CssClass="form-control" AutoUpdateAfterCallBack="true" ID="txtRut" runat="server" MaxLength="13"
                            TabIndex="3" onchange="validaRut(this);"></anthem:TextBox>
                    </div>

                </div>
                <div class="col-lg-4">
                    <div class='form-group'>
                        <label for="ddlEstado" class="text-left">Estado</label>
                        <anthem:DropDownList CssClass="form-control" AutoUpdateAfterCallBack="true" ID="ddlEstado" TabIndex="4" runat="server">
                        </anthem:DropDownList>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-lg-4">

                    <div class='form-group'>
                        <label for="ddlSucursal" class="text-left">Sucursal</label>
                        <anthem:DropDownList CssClass="form-control" AutoUpdateAfterCallBack="true" ID="ddlSucursal" TabIndex="4" runat="server">
                        </anthem:DropDownList>
                    </div>

                </div>
                <div class="col-lg-4">
                    <div class='form-group'>
                        <label for="ddlFabrica" class="text-left">Fábrica</label>
                        <anthem:DropDownList CssClass="form-control" AutoUpdateAfterCallBack="true" ID="ddlFabrica" TabIndex="4" runat="server">
                        </anthem:DropDownList>
                    </div>
                </div>



            </div>

            <div class="row">
                <div class="col-md-12 text-center">
                    <anthem:Button PreCallBackFunction="this.disabled = true" PostCallBackFunction="this.disabled = false" EnabledDuringCallBack="False" ID="btnBuscar" CssClass="btn-sm btn-primary" runat="server" Text="Buscar"
                        OnClick="btnBuscar_Click" TextDuringCallBack="Buscando..." />
                </div>
            </div>

            <div class="row">
                <anthem:Button PostCallBackFunction="this.disabled = false" EnabledDuringCallBack="False" ID="ImageButton2" CssClass="btn-success btn-sm" runat="server" Text="Nuevo Registro"
                    OnClick="btnNuevoRegistro_Click" />
                <div class="col-md-12">
                    <span class="badge bg-info">
                        <anthem:Label ID="lblContador" runat="server" AutoUpdateAfterCallBack="true"></anthem:Label></span>
                </div>
            </div>
            <div class="row">
                <div class="col-md-12">
                    <anthem:GridView runat="server" ID="gvBusqueda" AutoGenerateColumns="false" Width="100%"
                        PageSize="10" AllowPaging="True" AllowSorting="True" OnPageIndexChanging="gvBusqueda_PageIndexChanging"
                        AutoUpdateAfterCallBack="true" CssClass="table table-condensed">
                        <RowStyle CssClass="GridItem" />
                        <PagerStyle CssClass="GridPager" HorizontalAlign="Center" />
                        <HeaderStyle CssClass="GridHeader" HorizontalAlign="Center" />
                        <AlternatingRowStyle CssClass="GridAtlItem" />
                        <Columns>
                            <asp:BoundField DataField="Rut" HeaderText="Rut" />
                            <asp:BoundField DataField="Nombre" HeaderText="Nombre" />
                            <asp:BoundField DataField="DescripcionEstado" HeaderText="Estado" />
                            <asp:BoundField DataField="Email" HeaderText="Mail" />
                            <asp:TemplateField HeaderText="Modificar" ItemStyle-HorizontalAlign="Center">
                                <ItemTemplate>
                                    <anthem:ImageButton ID="btnModificar" runat="server" ImageUrl="~/Img/Grid/Editar_grid.png"
                                        ToolTip="Modificar" OnClick="btnModificar_Click" TextDuringCallBack="Buscando Registro..." />
                                </ItemTemplate>
                                <ItemStyle HorizontalAlign="Center"></ItemStyle>
                            </asp:TemplateField>
                        </Columns>
                    </anthem:GridView>
                </div>

            </div>
        </div>
        <h2 id="hFormulario" style="display: none;">
            <a id="lnkFormulario" href="#">Formulario</a></h2>
        <div>
            <div class="row">
                <div class="col-md-4">
                    <div class='form-group'>
                        <label for="txtFormNombreUsuario" class="text-left">Nombre de Usuario</label>
                        <anthem:TextBox CssClass="form-control" ID="txtFormNombreUsuario" runat="server" TabIndex="7"
                            AutoUpdateAfterCallBack="true" MaxLength="200"></anthem:TextBox>
                    </div>

                </div>

                <div class="col-md-4">
                    <div class='form-group'>
                        <label for="txtFormNombre" class="text-left">Nombre</label>
                        <anthem:TextBox CssClass="form-control" ID="txtFormNombre" runat="server" TabIndex="7" MaxLength="200"
                            AutoUpdateAfterCallBack="true"></anthem:TextBox>
                    </div>

                </div>

                <div class="col-md-4">
                    <div class='form-group'>
                        <label for="txtFormMail" class="text-left">Mail</label>
                        <anthem:TextBox CssClass="form-control" ID="txtFormMail" runat="server" TabIndex="9" onblur="validarEmail(this);"
                            AutoUpdateAfterCallBack="true" MaxLength="200"></anthem:TextBox>
                    </div>
                </div>
            </div>

            <div class="row">
                <div class="col-md-4">

                    <div class='form-group'>
                        <label for="txtFormRut" class="text-left">Rut</label>
                        <anthem:TextBox CssClass="form-control" ID="txtFormRut" runat="server" MaxLength="13" TabIndex="10" onchange="validaRut(this);"
                            onKeyPress="return SoloRut(event);" AutoUpdateAfterCallBack="true"></anthem:TextBox>
                    </div>
                </div>
                <div class="col-md-4">

                    <div class='form-group'>
                        <label for="ddlFormEstado" class="text-left">Estado</label>
                        <anthem:DropDownList CssClass="form-control" ID="ddlFormEstado" TabIndex="11" runat="server" AutoUpdateAfterCallBack="true">
                        </anthem:DropDownList>
                    </div>
                </div>
                <div class="col-md-4">

                    <div class='form-group'>
                        <label for="txtClave" class="text-left">Clave</label>
                        <anthem:TextBox CssClass="form-control" ID="txtClave" runat="server" TabIndex="7"
                            AutoUpdateAfterCallBack="true" Enabled="false"></anthem:TextBox>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-4">
                    <div class='form-group'>
                        <label for="txtFormTelefono" class="text-left">Teléfono</label>
                        <anthem:TextBox CssClass="form-control" ID="txtFormTelefono" runat="server"
                            onKeyPress="return SoloNumeros(event,this.value,0,this);" AutoUpdateAfterCallBack="true"></anthem:TextBox>
                    </div>
                </div>
                <div class="col-md-4">

                    <div class='form-group'>
                        <label for="ddlFormSucursal" class="text-left">Sucursal</label>
                        <anthem:DropDownList CssClass="form-control" ID="ddlFormSucursal" TabIndex="11" runat="server" AutoUpdateAfterCallBack="true">
                        </anthem:DropDownList>
                    </div>
                </div>

                <div class="col-md-4">
                    <div class='form-group'>
                        <label for="ddlFormFabrica" class="text-left">Fábrica</label>
                        <anthem:DropDownList CssClass="form-control" ID="ddlFormFabrica" TabIndex="11" runat="server" AutoUpdateAfterCallBack="true">
                        </anthem:DropDownList>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-4">
                    <div class='form-group'>
                        <label for="ddlFormInversionista" class="text-left">Inversionista</label>
                        <anthem:DropDownList CssClass="form-control" ID="ddlFormInversionista" TabIndex="11" runat="server" AutoUpdateAfterCallBack="true">
                        </anthem:DropDownList>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class='form-group'>
                        <label for="ddlFormInmobiliaria" class="text-left">Inmobiliaria</label>
                        <anthem:DropDownList CssClass="form-control" ID="ddlFormInmobiliaria" TabIndex="11" runat="server" AutoUpdateAfterCallBack="true">
                        </anthem:DropDownList>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class='form-group'>
                        <label for="txtMontoMaximoAprobacion" class="text-left">Monto Máximo de Aprobación</label>
                        <anthem:TextBox CssClass="form-control" AutoUpdateAfterCallBack="true" ID="txtMontoMaximoAprobacion" onKeyPress="return SoloNumeros(event,this.value,4,this);" runat="server"></anthem:TextBox>
                    </div>
                </div>

            </div>
            <div class="row">
                <div class="col-md-4">

                    <div class="panel panel-success">
                        <div class="panel-heading">
                            <h3 class="panel-title">Roles del Usuario</h3>
                        </div>
                        <div class="panel-body">
                            <anthem:ListBox ID="lstbFormRoles" runat="server" TabIndex="12" Width="100%" SelectionMode="Multiple"
                                AutoUpdateAfterCallBack="true" Rows="15">
                            </anthem:ListBox>
                        </div>
                    </div>
                </div>

            </div>
            <div class="row">
                <div class="col-md-12 text-center">
                    <anthem:Button PreCallBackFunction="this.disabled = true" PostCallBackFunction="this.disabled = false" EnabledDuringCallBack="False" ID="btnGuardar" runat="server" CssClass="btn-sm btn-primary" Text="Guardar"
                        OnClick="btnGuardar_Click" TextDuringCallBack="Guardando..." />
                    &nbsp;<anthem:Button PreCallBackFunction="this.disabled = true" PostCallBackFunction="this.disabled = false" EnabledDuringCallBack="False" ID="btnCancelar" runat="server" Text="Cancelar" CssClass="btn-sm btn-primary"
                        OnClick="btnCancelar_Click" TextDuringCallBack="Cancelando..." />
                </div>
            </div>
            <anthem:HiddenField ID="hfId" runat="server" Visible="False" AutoUpdateAfterCallBack="true" />

        </div>
    </div>
</asp:Content>
